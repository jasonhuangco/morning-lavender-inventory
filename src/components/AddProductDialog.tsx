...
const AddProductDialog: React.FC<AddProductDialogProps> = ({ open, onClose }) => {
  ...
  React.useEffect(() => {
    if (open) {
      ...
      const initialSettings: ProductLocation[] = state.locations.map(location => ({
        locationId: location.id,
        isAvailable: true,
        minThreshold: requiresQuantity ? 5 : undefined,
      }));
      setLocationSettings(initialSettings);
    }
  }, [open, state.locations, requiresQuantity]);
